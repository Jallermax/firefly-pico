...
- Run `.github/workflows/personal-sync.yml` manually or let its daily schedule call `scripts/sync-personal-branches.sh`. The script requires a clean checkout, merges without rebasing or force-pushing, tests and builds both candidates, and publishes both branches atomically only after every gate passes.
